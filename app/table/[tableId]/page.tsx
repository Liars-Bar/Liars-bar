...
export default function RetroTablePage() {
  ...
  // Dismiss decrypt popup once cards are successfully decrypted
  useEffect(() => {
    if (myCards.length > 0 && showDecryptPopup) {
      setShowDecryptPopup(false);
    }
  }, [myCards.length, showDecryptPopup, setShowDecryptPopup]);
  ...
  const playersWithPositions = playersWithCharacters.map(
    (playerInfo, index) => {
      ...
      // Find the player's index in the full players array to get their bullet count
      const playerIndex = tableData?.players.indexOf(playerInfo.address) ?? -1;
...
╚═══════════════════════════╝`}
          ...
          <a href="/" className="retro-btn retro-btn-amber">
            {"<< BACK TO MENU"}
          </a>
        </div>
      </div>
...
└───┘`}
        ...
        <div className="flex-1 relative min-w-0 min-h-0 overflow-hidden">
              ...
              .map((player) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Bullet chamber indicator */}
                    {player.bulletsLeft != null && !isEliminated && (
                      <div className="flex items-center gap-[2px] mt-0.5">
                        {Array.from({ length: 6 }).map((_, i) => (
                          <div
                            key={i}
                            className="w-[5px] h-[5px] sm:w-[6px] sm:h-[6px]"
                            style={{
                              borderRadius: "50%",
                              background: i < player.bulletsLeft!
                                ? "#fbbf24"
                                : "#222",
                              border: `1px solid ${i < player.bulletsLeft! ? "#f59e0b" : "#333"}`,
                              boxShadow: i < player.bulletsLeft!
                                ? "0 0 3px #fbbf2480"
                                : "none",
                            }}
                          />
          ...
          {/* ── Card Placed Animation — flies from player to table center ─── */}
          {activeAnimation?.type === "card-placed" && (() => {
            const sourcePlayer = playersWithPositions.find(
              (p) => p.address === activeAnimation.player,
            );
            // Determine start position: player seat or bottom-center for current player
            const startTop = sourcePlayer?.isCurrentPlayer
              ? "92%"
              : sourcePlayer?.top ?? "50%";
            const startLeft = sourcePlayer?.isCurrentPlayer
              ? "50%"
              : sourcePlayer?.left ?? "50%";
            const playerColor = sourcePlayer?.color ?? "#22d3ee";
            const playerName = sourcePlayer?.name ?? "???";

            return (
              <div className="absolute inset-0 z-40 pointer-events-none">
                {/* Flying cards (3 staggered) */}
                {[0, 1, 2].map((i) => (
                  <div
                    key={i}
                    className="absolute"
                    style={{
                      ...
                      animation: `card-fly-to-center 0.8s cubic-bezier(0.22, 0.61, 0.36, 1) ${i * 0.1}s forwards`,
                      opacity: 0,
                    }}
                  >
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: `0 0 12px ${playerColor}60, 3px 3px 0 #111`,
                        transform: `rotate(${(i - 1) * 8}deg)`,
                      }}
                    >
                      <span className="text-blue-300 text-[8px] opacity-40">?</span>
                    </div>
                  </div>
                ))}

                {/* Player name badge that follows the cards briefly */}
                <div
                  className="absolute"
                  style={{
                    top: startTop,
                    left: startLeft,
                    transform: "translate(-50%, -50%)",
                    animation: "card-fly-label 1s ease-out forwards",
                    opacity: 0,
                  }}
                >
                  <div
                    className="px-2 py-1 text-[7px] sm:text-[8px] whitespace-nowrap"
                    style={{
                      background: "#0a0a0aEE",
                      color: playerColor,
                      border: `2px solid ${playerColor}`,
                      boxShadow: `0 0 10px ${playerColor}50`,
                    }}
                  >
                    {playerName} PLAYED
                  </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    left: "50%",
                    transform: "translate(-50%, -50%)",
                    animation: "card-land-flash 0.6s ease-out 0.7s forwards",
                    opacity: 0,
                  }}
                >
                  <div
                    ...
                    style={{
                      background: `radial-gradient(circle, ${playerColor}40 0%, transparent 70%)`,
...
╚══════════════════════════════╝`}
      ...
      >
        {/* Decrypt Popup — retro style, shown after shuffle */}
        {showDecryptPopup && !isDecryptingCards && myCards.length === 0 && myEncryptedCards.length > 0 && (
          <div className="flex flex-col items-center gap-2 pixel-fade-in">
            <p className="neon-amber text-xs sm:text-sm font-bold tracking-widest">
              *** DECRYPT YOUR CARDS ***
            </p>
            <p className="text-green-500 text-[8px] sm:text-[10px]">
              CARDS SHUFFLED. SIGN TO REVEAL YOUR HAND.
            </p>
            <button
              onClick={() => decryptMyCards()}
              className="retro-btn retro-btn-amber text-[9px] px-6 py-2 mt-1"
            >
              SIGN & DECRYPT
            </button>
          </div>
        ...
        {isDecryptingCards && (
          <div className="flex flex-col items-center gap-2 pixel-fade-in">
            <p className="text-cyan-400 text-[9px] blink tracking-widest">
              DECRYPTING CARDS...
            </p>
          </div>
        )}
        ...
        {/* Cards */}
        {myEncryptedCards.length > 0 && myCards.length > 0 && (
          <div
      ...
      {connected && publicKey && myPlayer && (
        <div
          className="fixed z-50 left-3 p-2 flex flex-col items-center gap-2 panel-slide-in"
          style={{
            bottom: "5%",
            background: "#0a0a0aEE",
        ...
        >
          ...
          {/* Divider */}
          <div className="w-full" style={{ borderTop: `1px solid ${myPlayer.color}40` }} />
          ...
          <div className="flex items-center gap-2">
            <div
              ...
              style={{
                border: `2px solid ${myPlayer.color}`,
                boxShadow: `0 0 8px ${myPlayer.color}30`,
                imageRendering: "pixelated",
                background: "#0a0a0a",
              }}
            >
